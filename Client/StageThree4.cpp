...
StageThree4::StageThree4():
Scene("StageThreeFour")
{
}
...
void StageThree4::BuildScene()
{
	...
	LoadSceneDataStageThree(_T("Resources\\Json\\Stage 3-4.json"));

	// 플레이 스테이지들은 첫 번째로 로드될 때의 디폴트 정보를 저장한다.
	if (mIsFirstLoad)
	{
		mGamePlayer->GetTransform()->SetPosition(Vector2D(50.f, 500.f));				// 스테이지 3은 기획자분들이 정하는 위치로 간다.
