...
void StageThree1::BuildScene()
{
	...
	LoadSceneDataStageThree(_T("Resources\\Json\\Stage 3-1.json"));

	if (mIsFirstLoad)
	{
		mGamePlayer->GetTransform()->SetPosition(Vector2D(150.f, 100.f));				// 스테이지 3은 기획자분들이 정하는 위치로 간다.

		Game::GetInstance().SaveLastBodyWeight(mGamePlayer->GetBodyWeight());
